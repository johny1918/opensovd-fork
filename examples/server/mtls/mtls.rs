...
/*
    ...
    the local CA. Run mkcerts.sh first to generate the test certificates.

    Run with:
        cargo run -p opensovd-examples-server --example mtls --features tls

    Test with curl (client cert required):
        curl --cacert examples/server/mtls/ca.crt \
            --cert examples/server/mtls/client.crt \
            --key  examples/server/mtls/client.key \
...
// paths relative to workspace root; adjust if you move the certs.
const CERT: &str = "examples/server/mtls/server.crt";
const KEY: &str = "examples/server/mtls/server.key";
const CLIENT_CA: &str = "examples/server/mtls/ca.crt";
